...
export const businessKeys = {
  ...
  overview: (businessId) => [...businessKeys.detail(businessId), "overview"],
...
export const useOverviewData = (options = {}) => {
  ...
  const queryKey = isBusinessView 
    ? businessKeys.overview(business?.id)
    : branchKeys.overview(currentBranch?.id);
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (isBusinessView) {
        ...
        const response = await api.get(`/business/${business.id}/overview`);
        ...
        const response = await api.get(`/branches/branch/${currentBranch.id}/overview`);
